...
def bounds(FILE):
    pasta = "C:/Users/ravel/OneDrive/Área de Trabalho/DataScientist/sklearn/COVID-19/CasosPorEstado/GLM_covid19/data/range/"
    df_range = pd.read_csv(pasta+"/"+FILE, header = 0, sep = ";")
    p0 = df_range["p0"].values[:3]
    bsup = df_range["bsup"].values[:3]
    binf = df_range["binf"].values[:3]
...
def Sensitivity(mypath,FILE,pop):
    ...
    df_Y = pd.DataFrame(Y,columns = ['r', 'p', 'K','NSE'])
    df_W = pd.DataFrame(W,columns = ['r', 'p', 'K','NSE'])

    Si = sobol.analyze(problem,df_Y["NSE"].values)
    df_S = pd.DataFrame(Si["S1"], columns = ["S1"])
    df_S["ST"] = Si["ST"]
    df_S.to_csv("C:/Users/ravel/OneDrive/Área de Trabalho/DataScientist/sklearn/COVID-19/CasosPorEstado/GLM_covid19/data/sensibilidade/"+FILE,sep=";")

    binf = np.array([min(df_W.r),min(df_W.p), min(df_W.K)])
    bsup = np.array([max(df_W.r),max(df_W.p), max(df_W.K)])
    ...
    p0 = np.array([df_W.r[pos_max],df_W.p[pos_max],df_W.K[pos_max]])

    par = np.array([binf,bsup,p0])

    df_par = pd.DataFrame(par, ["binf","bsup", "p0"]).transpose()
    df_par.to_csv("C:/Users/ravel/OneDrive/Área de Trabalho/DataScientist/sklearn/COVID-19/CasosPorEstado/GLM_covid19/data/range/"+FILE,sep=";")
...
def object_minimize(x,t,cumdata_cases):
    ...
    cum_cases = odeint(f,x[3],t, args =(x[0],x[1],x[2]))
...
def min_minimize(cumdata_cases,C,p0,t,bsup,binf):
    bnds = ((binf[0],bsup[0]),(binf[1],bsup[1]),(binf[2],bsup[2]),(binf[3],bsup[3]))
...
mypath = "C:/Users/ravel/OneDrive/Área de Trabalho/DataScientist/sklearn/COVID-19/CasosPorEstado/GLM_covid19/data/data_mensured"
...
df_R = pd.DataFrame(R, columns = ['r','p','K','NSE','RMSE','MARE'])
df_R["Estado"] = estados
path_out ="C:/Users/ravel/OneDrive/Área de Trabalho/DataScientist/sklearn/COVID-19/CasosPorEstado/GLM_covid19/"
...
def bar_plt(atributo, title_name,df_R,logscale):
    ...
    plt.show()
    path_out ="C:/Users/ravel/OneDrive/Área de Trabalho/DataScientist/sklearn/COVID-19/CasosPorEstado/GLM_covid19/imagens/"
